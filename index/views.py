...
def app_page(request):
    # نمایش صفحه HTML با دکمه دانلود
    return render(request, 'root/app.html')

def download_app(request):
    # مسیر فایل app.apk کنار views.py در پوشه app
    file_path = os.path.join(os.path.dirname(__file__), 'app', 'app.apk')

    if not os.path.exists(file_path):
        raise Http404("فایل یافت نشد")

    try:
        return FileResponse(open(file_path, 'rb'), as_attachment=True, filename='app.apk')
    except Exception as e:
        print(f"Error downloading app.apk: {e}")
        raise Http404("خطا در دانلود فایل")
